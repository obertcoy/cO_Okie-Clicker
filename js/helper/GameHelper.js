...
import { checkClickUpgrade, displayUpgrade, getUnlockedUpgrades, upgradeUpdate } from "./UpgradeHelper.js";
...
export function initBuildingButton() {
    ...
    buildings.forEach((building, index) => {
        if (currentUser.hasBuilding(building.type))
            buildings[index] = currentUser.getBuilding(building.type);
    });
    buildings.forEach((building, index) => {
...
export function displayCookie(userCookieDisplay, userCpsDisplay, currentUser) {
...
export function increaseCookie(lastTimestamp, timestamp) {
    if (lastTimestamp === 0)
...
export function initUnlockedUpgrade() {
    ...
    unlockedUpgrades.map((upgrade) => {
        if (upgrade.subcontainer.classList.contains("hidden") && !Game.getCurrentUser().hasUpgrade(upgrade))
            upgrade.subcontainer.classList.remove("hidden");

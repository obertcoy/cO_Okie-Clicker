...
export function createHBorder() {
    const hBorder = document.createElement("div");
    hBorder.className = "h-border";
    return hBorder;
}
export function createUnavailableOverlay() {
    const overlay = document.createElement("div");
    overlay.className = "unavailable";
    return overlay;
}
export function displayStoreInfo(element, eventY) {
    var _a, _b;
    const container = document.getElementById("store-info");
    const img = document.getElementById("store-info-img");
    ...
    if (element instanceof Upgrade) {
        ...
        if (Game.getCurrentUser().getBuilding(element.type)) {
            ...
            info1.textContent = `each ${getBuildingTypeName(element.type)} produces ${(((_a = Game.getCurrentUser().getBuilding(element.type)) === null || _a === void 0 ? void 0 : _a.baseCps) * ((_b = Game.getCurrentUser().getBuilding(element.type)) === null || _b === void 0 ? void 0 : _b.multiplier)).toFixed(1)} per second`;
...
export function displayInfo(element, posX, posY, width, unlocked = true) {
...
export function timeAgo(runStartedTimestamp) {
    const now = Date.now();
...
export function changeNameOverlay() {
    ...
    confirmBtn.addEventListener("click", () => {
        var _a;
        Game.getCurrentUser().setUsername((_a = nameInput.value) !== null && _a !== void 0 ? _a : userNameContainer.textContent);
...
export function cookieShower(container, currentUser) {
    const cookieShower = document.createElement("img");

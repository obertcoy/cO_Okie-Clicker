...
export const buildingsImages: BuildingImage[] = [
  ...
  export function addBuildingButton(building: Building, buildingImage: BuildingImage) {
    ...
    if(storeBuildingContainer) storeBuildingContainer.appendChild(buildingContainer)
    ...
    building.sm?.enterState()
  }

  export function buyBuilding(building: Building, user: User): void {

    if(user.getCookies() < building.getCurrentPrice()) return
    // if(!user.hasBuilding(building.type)) user.buildings.push(building)
    if(building.type === BuildingType.Grandma) createGrandma(user)

    building.buy(user)
    buildingUI(building)
    checkUnlockUpgrade(building.type, user)
    checkAchievementCondition(AchievementType.Building)
    displayStoreInfo(building, building.ui?.posY ?? 0)
    updateStatsImages()
    saveData(user)
}

export function getBuildingTypeName(type: BuildingType){
  return BuildingType[type].toString()
}

export function createGrandma(user: User){
